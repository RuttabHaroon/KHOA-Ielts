//
//  TestRecordingScreenViewController.swift
//  Khoa IELTS
//
//  Created by Office on 7/26/19.
...
class TestRecordingScreenViewController: UIViewController, AVAudioRecorderDelegate, AVSpeechSynthesizerDelegate {
    ...
    var zeroTransitionSentence = "Hello, can you tell me your full name, please?\n\n"
    var firstTransitionSentence = "Can i see your identification please? Thank you.\n\nNow, in the first part, I'd like to ask you some question about yourself.\n\n"
    var secondTransitionSentence = "Now, I'm going to give you a topic, and I'd like you to talk about it for one to two minutes. Before you talk, you'll have one minute to think about what you're going to say. You can make some notes if you wish... Here's some paper and a pencil for making notes, and here's your topic"
    var thirdTransitionSentence = "Now, I'd like to discuss with you a few more general questions"
    
    
    var part0 = Bool()
    var part0Intialize = Bool()
    var part1 = Bool()
    var part1Initialize = Bool()
    var part3 = Bool()
    var part3Initialize = Bool()
    ...
    @IBOutlet weak var part2ViewText: UILabel!
...
//        if isTestNow {
//            data.removeAll()
//            data = questionsWithMultipleParts[1]
//        }
//
//
...
//        if currentQuestionPart == 1 {
//            //loadNextQuestionPart()
//        }
...
//        }
        ...
        if globalIndex == topicCount && currentQuestionPart <= 1 {
            makeEmptyState(visibile: true)
        }
        ...
        if (globalIndex == topicCount && currentQuestionPart <= 1) {
            ...
            startRecordingTapped(self.nextQuesBtn)
            return
...
//      }
        ...
        if !startRecordingTimer.isValid {
    ...
    @objc func readSentence () {
        ...
        if !part2Initialize  && !part1Initialize && !part0Intialize {
            noteFrequencies.append(currentFrequency)
            finishRecording(true)
            return
        }
        ...
        if part2 {
            let speech   = AVSpeechUtterance(string: secondTransitionSentence)
        ...
        if part1 {
            let speech   = AVSpeechUtterance(string: firstTransitionSentence)
            speech.voice = AVSpeechSynthesisVoice(language: "en-US")
            //speechSynthesizer.speak(speech)
            queuePlayerPlay()
            return
        ...
        if part3 {
            let speech   = AVSpeechUtterance(string: thirdTransitionSentence)
            speech.voice = AVSpeechSynthesisVoice(language: "en-US")
            //speechSynthesizer.speak(speech)
            queuePlayerPlay()
            return
        ...
        if part0 {
            let speech   = AVSpeechUtterance(string: zeroTransitionSentence)
            speech.voice = AVSpeechSynthesisVoice(language: "en-US")
            //speechSynthesizer.speak(speech)
            queuePlayerPlay()
            return
...
//            }
            ...
            if value.contains("?") {
                if title  != "Personal Matters & Hobbies" {
                    ...
                    var topicTitle = data["topicTitle"] as! String
                    if topicTitle != "Arrive on Time" || topicTitle != "Books or TV" {
                        value = value.replacingOccurrences(of: "?", with: "")
                    }
                }
            }            
...
//            if part2QuestionRead == false {
//                part2QuestionRead = true
//                //Ruttab.  I commented this because audio for part 3 wasnt playing in Practice option so it also shouldnt play in Test option
...
//                }
                ...
                callTextToSpeech = Timer(timeInterval: TimeInterval(recordingTimer), target: self, selector: #selector(readSentence), userInfo: nil, repeats: true)
                updateSoundWaves = Timer.scheduledTimer(timeInterval: 0.009, target: self, selector: #selector(refreshAudioView(_:)), userInfo: nil, repeats: true)
                print("60 seconds has passed Start timer for question 2")
            }
        }
        
        if part1 || part3 {
            // part1Initialize = false
            part1 = false
        ...
        if recordingTimer < 59 && part2 {
            countdownTimer.adjustsFontSizeToFitWidth = true
            countdownTimer.text = "You have 1 minute to brainstorm your answer."
        } else if recordingTimer >= 59 && recordingTimer < 60 && part2 {
            countdownTimer.adjustsFontSizeToFitWidth = true
            countdownTimer.text = "You can start now!"
        } else {
            countdownTimer.adjustsFontSizeToFitWidth = false
...
extension TestRecordingScreenViewController : UITableViewDelegate, UITableViewDataSource {
...
extension TestRecordingScreenViewController : SFSpeechRecognizerDelegate, SFSpeechRecognitionTaskDelegate {
...
extension TestRecordingScreenViewController {
    @available(iOS 11.0, *)
...
//        let prband = (Double(totalwords + frequencyCounter) / 2) * 0.25
         ...
         * IELTS score are rouned up to 0.5  e.g 0.5 , 1.0, 1.5,  2.0
         */
...
extension TestRecordingScreenViewController {
    func intialization() {
        ...
        data = questionsWithMultipleParts[0]
        ...
        //This is done because in first part  of ilets only 4 question from a group of topics is asked
        //therefore, i am removing all but 4 question from data["topics"]
        if topicCount > 4 {
            var tempArr = data["topics"] as! Array<String>
            let  range = 4..<topicCount
            tempArr.removeSubrange(range)
            
            data["topics"] = tempArr as AnyObject
        ...
        part2ViewText.text = zeroTransitionSentence //firstTransitionSentence
        ...
        //setFirstTransitionVoiceovers()
        setTransitionVoiceovers()
    ...
    func loadNextQuestionPart() {
        ...
        if currentQuestionPart == 1 {
            ...
            part2 = true
            part2Initialize = true
            self.part2ViewText.text = secondTransitionSentence
            setTransitionVoiceovers()
            ...
            //RUTTAB
            if self.countdownTimer.isHidden == false {
                self.audioWave.isHidden = true
                self.countdownTimer.isHidden = true
            }
            //END RUTTAB
            
        } else if currentQuestionPart == 2 {
            part3 = true
            part3Initialize = true
            self.part2ViewText.text = thirdTransitionSentence
            part2View.isHidden = true
            tableViewReloader = 1
    ...
    func loadFirstPart() {
        ...
        part1 = true
        part1Initialize = true
        self.part2ViewText.text = firstTransitionSentence
        tableViewReloader = 1
        nextQuesBtn.isEnabled = true
        nextQuesBtn.alpha = 1
        setTransitionVoiceovers()
...
//        let path = Bundle.main.path(forResource: "\(topic[globalIndex]).wav", ofType:nil)!
...
//    }
    ...
    func playAudioFile(audioUrl: String) {
        ...
        audioPath.downloadURL { [weak self] (url, err) in
            ...
            if err != nil {
                ...
                weakSelf.displayAudioErrorAlert(err: "Something went wrong. Audio file not played!")
            }
    ...
    func setTransitionVoiceovers() {
        
        var urls: [URL] = []
        
        if part0 {
            urls.append(URL(fileURLWithPath: Bundle.main.path(forResource: "TR01 - Intro.wav", ofType:nil)!))
        }
        
        if part1 {
            //urls.append(URL(fileURLWithPath: Bundle.main.path(forResource: "TR01 - Intro.wav", ofType:nil)!))
            urls.append(URL(fileURLWithPath: Bundle.main.path(forResource: "TR02 - Intro.wav", ofType:nil)!))
            urls.append(URL(fileURLWithPath: Bundle.main.path(forResource: "TR03 - Before Part 1.wav", ofType:nil)!))
        }
        
        if part2 {
            urls.append(URL(fileURLWithPath: Bundle.main.path(forResource: "TR04 - Before Part 2.wav", ofType:nil)!))
        }
        
        if part3 {
            urls.append(URL(fileURLWithPath: Bundle.main.path(forResource: "TR05 - Before Part 3.wav", ofType:nil)!))
        }
        
        for url in urls {
            makeAudioAssetItem(url: url)
        }
    }
    
    func makeAudioAssetItem(url: URL) {
...
//        }
    ...
    func queuePlayerPlay() {
        
        print("Items in qplayer \(self.queuePlayer.items().count)")
        
        self.queuePlayer.play()
        NotificationCenter.default.addObserver(self,
                                               selector: #selector(self.queuePlayerDidFinishPlaying),
                                               name: NSNotification.Name.AVPlayerItemDidPlayToEndTime,
                                               object: self.queuePlayer.items().last)
    }
    
    @objc func queuePlayerDidFinishPlaying() {
        
        print("PLSGUYGGJH")
        
        makeEmptyState(visibile: false)
        
        if part2 || part1 || part3 {
            
            //RUTTAB
            if self.countdownTimer.isHidden == true {
                self.audioWave.isHidden = false
    ...
    func createAudioUrl() -> String {
       ...
       if part2 {
            ...
            if title  == "Food" || title == "TV, Media & Entertainment" {
                ...
                if topicTitle == "Helper Person" || topicTitle == "Helpful person" || topicTitle == "Someone Does Well" || topicTitle == "Well doing Person" || topicTitle == "Visitors to your home?"
                 || topicTitle == "Gift or Presents" || topicTitle == "Family" {
                    
                    if topicTitle == "Gift or Presents" || topicTitle == "Family" {
    ...
    func makeEmptyState(visibile: Bool) {
        if visibile == true {
...
//RUTTAB
//extension TestRecordingScreenViewController: AVAudioPlayerDelegate {
